...
int maxSequentially(const int arr[], const size_t n)
{
    ...
    if (n == 0 || arr == NULL)
    {
        printf("Невозможно провести операцию нахождения суммы подряд идущих элементов так как массив имеет длину 0.\n");
    }
    else if (k == 0 || k > n)
    {
        printf("Невозможно провести операцию нахождения суммы подряд идущих элементов из-за несоответствия длины скользящего окна k и длинны массива n.\n");
    }
    else
    {
        for (i = 0; i < n - k + 1; i++)
