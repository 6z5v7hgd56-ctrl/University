...
# def main():
#     xs = np.linspace(-1.5, 0.5, num=1000)
#     ys = np.linspace(-1.0, 1.0, num=1000)
...
def main():
    ...
    xs = np.linspace(-1.5, 0.5, num=size)
    ys = np.linspace(-1.0, 1.0, num=size)
    xs, ys = np.meshgrid(xs, ys)
    zs = xs + ys*1.j
    ws = np.zeros(shape=zs.shape, dtype=complex)

    extent = np.zeros(shape=xs.shape, dtype=int)

    for e in range(0, 100):
        ws = ws*ws + zs
        extent[np.abs(ws) > upper_bound] = e
